...
void SceneMenu::render()
{
	...
	if (myMenu.levelSelecting)
	{
		...
		for (int i = 0; i < 7; ++i)
		{
			RenderHelper::getInstance()->texture(myMenu.buttonSelect[i], myMenu.buttonSelectX[i], myMenu.buttonSelectY[i], myMenu.buttonWidth * myMenu.buttonSelectScale[i], myMenu.buttonHeight * myMenu.buttonSelectScale[i], 1, Color{ 0,0,0,1 }, 0, Database::getInstance().data["levels"][i + 1]["unlocked"] ? Color{ 1,1,1,1 } : Color{ 0.2f, 0.2f, 0.2f, 1 });
		}
